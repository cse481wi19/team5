...
class Base(object):
    ...
    def __init__(self):
        if REAL_ROBOT:
            ...
            self.odom_sub = rospy.Subscriber('odom',nav_msgs.msg.Odometry,callback=self._odom_callback,queue_size=10)
        else:
            self._publisher = rospy.Publisher('mobile_base_controller/cmd_vel', geometry_msgs.msg.Twist, queue_size=5)
            self.odom_sub = rospy.Subscriber('mobile_base_controller/odom',nav_msgs.msg.Odometry,callback=self._odom_callback,queue_size=10)
        self.odom = None
    ...
    def turn_al(self, angular_distance, speed=1):
