...
class FaceBehavior(object):
    ...
    def moveToTalk(self):
        ...
        # self.talk_position = self.nav_client.current_pose
        ...
        P_in_B = np.array([self.face_point.point.x - 2, self.face_point.point.y, self.face_point.point.z, 1]).reshape(4, 1)
        ...
        # t1.start()
        self.nav_client.send_waypoint(x_rob, y_rob, np.sin(theta/2), w)

    def moveBackpack(self):
        if self.face_point is None:
            return
        point_frame = self.face_point.header.frame_id
        trans, rot = None, None
        ...
        P_in_B = np.array([self.face_point.point.x - 0.5, self.face_point.point.y, self.face_point.point.z, 1]).reshape(4, 1)
        P_in_A = np.dot(T_A_B, P_in_B)
        x_rob, y_rob, z_rob = P_in_A[0], P_in_A[1], P_in_A[2]
    ...
    # def returnToTalk(self):
    #     if self.talk_position is None:
    #         return
    #     x_rob = self.talk_position.pose.position.x
    #     y_rob = self.talk_position.pose.position.y
    #     z = self.talk_position.orientation.z
    #     w = self.talk_position.orientation.w
    #     self.nav_client.send_waypoint(x_rob, y_rob, z, w)

    # def moveAway(self):
    #     x_rob = self.nav_client.current_pose.x
    #     y_rob = self.talk_position.pose.position.y
    #     z = self.talk_position.orientation.z
    #     w = self.talk_position.orientation.w
    #     self.nav_client.send_waypoint(x_rob, y_rob, z, w)
